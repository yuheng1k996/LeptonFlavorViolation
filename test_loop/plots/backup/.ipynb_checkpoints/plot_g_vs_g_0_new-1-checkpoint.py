...
def ReadLine(resdir, fillname, key):
#     key = key + ":"
  ...
  for resdir in resdircollection.split(":"):
    #os.chdir(resdir)
        ...
        for filename in os.listdir(resdir):
            ...
            for k in keys:
                if "(ind. and cum.)" in k:
                    test = ReadLine(resdir, fillname, k)
...
#                     KEYS.append(k[:k.find("(ind. and cum.)")] + "(cum.)")
                    ...
                    newdata[0,ndata] = ReadLine(resdir, fillname, k)
#                     KEYS.append(k)
                    ndata +=1
            #print(filename)
            #counter = counter + 1
...
filename = "g_vs_g_0_prompt"

#plot signal
plot_logx_logy_logz(data1[:,KEYS.index("g_CC_L:")], xlabel, data1[:,KEYS.index("g_AB_L:")], ylabel, data1[:,KEYS.index("Prompt_reallyobservedX: ")], zlabel, title, filename, 'k','-',r'$m_{X} = 1.5 \: \textrm{GeV}$', 3 , 1 ,[(0.8,5e-8)],'3signal')
plot_logx_logy_logz(data2[:,KEYS.index("g_CC_L:")], xlabel, data2[:,KEYS.index("g_AB_L:")], ylabel, data2[:,KEYS.index("Prompt_reallyobservedX: ")], zlabel, title, filename, 'r','-',r'$m_{X} = 1.0 \: \textrm{GeV}$', 3 , 1 ,[(0.8,5e-8)],'3signal')
plot_logx_logy_logz(data3[:,KEYS.index("g_CC_L:")], xlabel, data3[:,KEYS.index("g_AB_L:")], ylabel, data3[:,KEYS.index("Prompt_reallyobservedX: ")], zlabel, title, filename, 'c','-',r'$m_{X} = 0.5 \: \textrm{GeV}$', 3 , 1 ,[(0.8,5e-8)],'3signal')
