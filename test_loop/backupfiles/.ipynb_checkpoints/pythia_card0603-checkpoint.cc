...
bool pythia_card::doCalculations() {
    ...
    reallyProducedX = number_total_tau_tau * (2 * BrTau2Leptons * (BRtau2xmu(mX, g_TM_L, g_TE_L) + BRtau2xe(mX, g_TM_L, g_TE_L)));
...
bool pythia_card::runPythia(int nEventsMC) {
			...
			{	//count x
				producedX += 1;
                    ...
                    if(l1*l2){
                        ...
                        //record the values of all d0 and z0
                        //myfile << "d1_" << d1 << "_z1_" << z1 << "_d2_" << d2 << "_z2_" << z2 << '\n';
    ...
    std::cout << '\n';
    std::cout << "cut flow:" << '\n';
    std::cout << "nProduced x: "<< producedX << '\n';
    std::cout << "nFidVol: " << observedX << '\n';
    std::cout << "nd0z0: " << nd0z0 << '\n';
